...
def sample_sequence(train_data, sequence_length,
    ...
    for key, input_arr in train_data.items():
        ...
        print('key:', key)  # agent_pos, action, image
        print("sample.shape:", sample.shape)  # (16, 2), (16, 2), (16, 3, 96, 96)
        data = sample
        ...
        print("data.shape:", data.shape)  # (16, 2), (16, 2), (16, 3, 96, 96)
        result[key] = data
    return result
...
class PushTImageDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        print("episode_ends.shape:", episode_ends.shape)  # (206,)
        ...
        indices = create_sample_indices(
            ...
            pad_before=obs_horizon-1,
            pad_after=action_horizon-1)
        print("indices.shape:", indices.shape)  # (24208, 4)
        ...
        print("train_image_data:", train_image_data[0, :, :, :])
    ...
    def __getitem__(self, idx):
        ...
        print('nsample.shape:', nsample['image'][:self.obs_horizon,:].shape)  # (2, 3, 96, 96)
        print('nsample.shape:', nsample['agent_pos'][:self.obs_horizon,:].shape)  # (2, 2)
        print('nsample.shape:', nsample['action'][:,:].shape)  # (16, 2)
        return nsample
...
if __name__=="__main__":
    ...
    noise_pred_net = ConditionalUnet1D(
        ...
        global_cond_dim=obs_dim * obs_horizon  # (514*2+514+10) * 2
    )
    ...
    with torch.no_grad():
        ...
        # print("obs.shape:", obs.shape)  # torch.Size([1, 2, 514])
    ...
    if command == "train":
        ...
        with tqdm(range(num_epochs), desc='Epoch') as tglobal:
            ...
            for epoch_idx in tglobal:  # tglobal是一个变量名，它是用于引用tqdm迭代器的，epoch_idx=0,1,2,...
                epoch_loss = list()
                # nets.load_state_dict(torch.load('./model/ema_model_95.pth'))
                # _ = nets.to(device)
                ...
                with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
                    ...
                    for nbatch in tepoch:
                        ...
                        # 预测噪声残差
                        noise_pred = noise_pred_net(sample=noisy_actions, timestep=timesteps, global_cond=obs_cond)
    ...
    if command == "test":
        ...
        ema_nets.load_state_dict(torch.load('./model_save/ema_model_0.pth'))
